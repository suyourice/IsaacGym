...
from .pointfoot_jump_config import BipedCfgJump

class BipedJump(BaseTask):
    
    def __init__(
        self, cfg: BipedCfgJump, sim_params, physics_engine, sim_device, headless
    ):
    ...
    # --------------------------- reward functions---------------------------
    # YOU-RI: Rewards for jumping
    def _reward_jump_height(self):
        # Reward for jumping higher than target
        base_height = self.root_states[:, 2]  # robot base z position
        reward = torch.clip(base_height - self.cfg.rewards.base_height_target, min=0.0)
        return reward
